...
export async function handleIngestion(): Promise<void> {
  ...
  try {
    ...
    if(!chunkedDocuments || chunkedDocuments.length === 0) {
        logger.error('No documents were chunked. Please check the source documents.');
        process.exit(1);
    } else {
        logger.info(`Successfully split documents into ${chunkedDocuments.length} chunks.`);
    }
 ...
 * @returns 
 */
async function loadDocumentsFromDirectory(files: string[]): Promise<Document[]> {
